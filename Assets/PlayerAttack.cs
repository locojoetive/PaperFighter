...
public class PlayerAttack : MonoBehaviour
{
        ...
        switch (attackState)
        {
            ...
            case AttackState.DONE:
                if (InputManager.action && state.recovered)
                {
                    ...
                    state.audioManager.PlaySound("shurikenSpawn", 1F);
                    attackCount++;
                }
                break;
    ...
    public void SpawnShuriken()
    {
        ...
        state.animator.SetBool("ReleaseAttack", false);
        state.audioManager.PlaySound("shurikenShoot", 1F);
        Destroy(aimClone);
    ...
    private void KeepAiming()
    {
        ...
        shootDirectionLocalSpace = NormalizeVector(shootDirectionLocalSpace);
